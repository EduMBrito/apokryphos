...
export default async function TagPage({ params }: TagPageProps) {
  // 1. O Guardião: Verifica a sessão
  const session = await getServerSession(authOptions);
  if (!session) {
    notFound(); // Se não estiver logado, a página finge que não existe
  }

  // Decodifica a URL (caso a tag tenha caracteres especiais)
  const decodedTagName = decodeURIComponent(params.name).toLowerCase();

  // 2. Busca todos os posts que não estão deletados e que possuem essa tag
  ...
  return (
    <main className="max-w-4xl mx-auto py-12 px-6 min-h-screen">
      <div className="mb-8">
        ...
        <h1 className="text-3xl md:text-4xl font-serif text-slate-100 flex items-center gap-3">
          <span className="text-indigo-400 font-sans font-light">#</span>
          {decodedTagName}
        </h1>
        <p className="text-slate-400 mt-2">
          {posts.length} {posts.length === 1 ? "sedimento encontrado" : "sedimentos encontrados"}
        </p>
      </div>

      <section className="space-y-6">
        {posts.length === 0 ? (
          <p className="text-slate-400 italic">Nenhum pensamento encontrado com esta marcação.</p>
        ) : (
